...
def clusters2links(clusters, parses):  # 80224 Turtle 4 [x] 80228 replaced FIXME:DEL
...
def cluster_connectors(clusters, parses):  # 80228 [x] 80313 replaced FIXME:DEL
...
def cluster_links(clusters, links):  # 80313 Turtle 4 cluster_connectors replacement
    # 80224 Turtle 4 clusters2links - 80228 replaced by cluster_connectors
    cluster_links = links.groupby(['link']).sum().reset_index().copy()
    cluster_words = {row[1]: row[2] for row in clusters.itertuples()}
    def words(connector):
        x = []
        x.append(cluster_words[connector[:3]])
        x.append(cluster_words[connector[3:]])
        return x
    cluster_links['linked_clusters'] = cluster_links['link'].apply(words)
    linked_clusters = {row[1]: row[3] for row in cluster_links.itertuples()}
    return linked_clusters
