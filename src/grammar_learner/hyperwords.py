...
def vector_space_dim(links, path, tmpath, dim_max=100, sv_min=0.9, \
    ...
    logger = logging.getLogger(__name__ + ".vector_space_dim")
    #-%matplotlib inline
    ...
    logger.info(f'Singular values ({str(len(sv))}):{", ".join(str(round(x,1)) for x in sv)}')
    ...
    logger.warning(f'Vector space dimensionality = {dim+1}')
    logger.warning(f'Singular value ({str(dim+1)}) = {round(sv[dim],1)}')
    logger.warning(f'Max singular value ({str(1)}) = {round(sv[0],1)}')
    if dim+1 < len(sv):
        logger.warning(f'Singular value ({str(dim+1)}) = {sv[dim+1]}')
    else:
        logger.warning(f'All singular values within relevance interval - more than {sv_min} of max singular value')
        #+str(round((max(sv)*sv_min), 2))+')'
