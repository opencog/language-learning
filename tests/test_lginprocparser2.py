...
from src.grammar_tester.lginprocparser2 import LGInprocParser2, LGParseQualityEstimator, LGPSStreamParser
...
class LGInprocParser2TestCase(unittest.TestCase):
    def setUp(self):
        setup_logging(logging.DEBUG, logging.DEBUG, "/var/tmp/parse/LGInprocParser2TestCase.log", "w")
    ...
    def test_parse_sent_count(self):
        ...
        options = BIT_PARSE_QUALITY | BIT_EXISTING_DICT | BIT_LG_EXE | BIT_NO_LWALL #| BIT_OUTPUT_DIAGRAM

        pm, pq = pr.parse("tests/test-data/dict/poc-turtle", "tests/test-data/corpora/poc-turtle/poc-turtle.txt",
                 "/var/tmp/parse", "tests/test-data/parses/poc-turtle-mst/poc-turtle-parses-expected.txt", options, bar)
